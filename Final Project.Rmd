...
##get the list of names for companies that have warning letters, only for year 2019
companies_2019 <- warningletter %>%
  filter(warningletter$`Letter Issue Date`>=as.Date("2019-01-01")) %>% 
  select(`Company Name`)

companies_2019 <- unique(companies_2019)
head(companies_2019)
...
database_flat <- lapply(1:length(database_recalls_2019), function(x) database_recalls_2019[[x]][-c(14)]) %>% rbindlist(fill = T)
